...
window.onload=function()
{
	...
	var now=0;
	aBig.style.height=aBigLi[0].offsetHeight*aBigLi.length+'px';
	...
	function tab()
	{
		startMove(aBig,'top',-now*aBigLi[0].offsetHeight)
		if(now==0)
		{
			startMove(aSmall,'top',0)
		}
		else if(now==aSmallLi.length-1)
		{
			startMove(aSmall,'top',-(now-3)*(aSmallLi[0].offsetWidth+5))
		}
		else if(now==aSmallLi.length-2)
		{
			startMove(aSmall,'top',-(now-2)*(aSmallLi[0].offsetWidth+5))
		}
		else
		{
			startMove(aSmall,'top',-(now-1)*(aSmallLi[0].offsetWidth+5));
		}
		for(var i=0;i<aSmallImg.length;i++)
		{
			...
			aSmallImg[i].onmouseout=function()
			{
				if(now!=this.index)
				{
                    startMove(aSmallImg[now],'opacity',50);
					startMove(this,'opacity',100)
				}
			}	
		}
